...
def returnTestInput():
    ...
    if v.get() == 1:
        ...
        tmp = Label(window, text='Naive Bayes').grid(row=3,column=1)
        labels.append(tmp)
        tmp = Label(window, text=f'Accuracy : {accuracy[0]}%').grid(row=4,column=1 )
        labels.append(tmp)
        tmp = Label(window, text=f'Confusion Matrix : ').grid(row=5,column=1 )
        labels.append(tmp)
        tmp = Label(window, text=f'{confusion_matrix[0][0]}').grid(row=6,column=1 )
        labels.append(tmp)
        tmp = Label(window, text=f'{confusion_matrix[0][1]}').grid(row=7,column=1 )
        labels.append(tmp)

        tmp = Label(window, text='').grid(column=2)
